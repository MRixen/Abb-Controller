MODULE PvCom
  ...
  PROC main()
  	WHILE TRUE DO
  		bPvComReady:=TRUE;
  		WHILE DOutput(DOF_InitPVCom)=0 DO
  			Waittime 0.2;
  			IF bPvComReady=FALSE bPvComReady:=TRUE;
  		ENDWHILE
	    InitTraps;
	    Reset DOF_InitPVCom;
	    WHILE Doutput(DOF_InitPVCom)=0 AND bPvComReady=TRUE DO
  ...
  TRAP CoordTrap
  IF DOutput(showPVcmts)=1 THEN
  	TPWrite "CoordTrap für " +ValToStr(cam);
	ENDIF
    !
	...
	IF DOutput(showPVcmts)=1 THEN
    	TPWrite "let the other robot tasks take";
		ENDIF
      RETURN;
    ENDIF
    !
	...
	IF DOutput(showPVcmts)=1 THEN
    	TPWrite "cam=0 & IS_SLAVE_PVMCSYS=TRUE";
		ENDIF
      RETURN;
    ENDIF
    !
    ...
    IF bCoordReceived{cam}=TRUE THEN
	  ...
	  IF DOutput(showPVcmts)=1 THEN
      TPWrite "we cannot receive coordinates f";
	  ENDIF
  ...
  PROC InitTraps()
    ...
    IF BELT_ACTION{CAMERA_NO_1}<>RUN_NEVER THEN
	  ...
	  IF DOutput(showPVcmts)=1 THEN
      TPWrite "Interrupt Band1 aktiviert";
	  ENDIF
    ENDIF
    IF BELT_ACTION{CAMERA_NO_2}<>RUN_NEVER THEN
	  ...
	  IF DOutput(showPVcmts)=1 THEN
      TPWrite "Interrupt Band2 aktiviert";
	  ENDIF
    ENDIF
