MODULE Articles
    ...
    VAR string filename:="Home:/Diagnose/Protocols/LastUsedArticles.txt";
    ...
    PROC setArticlesAndCounters()
        ...
        FOR row FROM 1 TO MAX_ARTICLE_COUNTER DO
		    lastArticleDate{row}:=readArticleData(filename,row,1);
            lastArticleTime{row}:=readArticleData(filename,row,2);
            lastArticleNames{row}:=readArticleData(filename,row,3);
            lastArticleCounters{row}:=readArticleData(filename,row,4);
            lastArticleCycleTime{row}:=readArticleData(filename,row,5);
        ENDFOR
    ...
    PROC sendArticleData()
        IF (articleCounter>0) THEN
            ...
            ! Set header
            saveArticleData filename,data,header,MAX_ARTICLE_DESCRIPTION_DATA,TRUE,TRUE,FALSE;
            ...
            FOR j FROM 1 TO MAX_ARTICLE_COUNTER DO
                ...
                IF (j<=articleCounter) THEN
                    IF (j=MAX_ARTICLE_COUNTER) THEN
                        saveArticleData filename,data,header,MAX_ARTICLE_DESCRIPTION_DATA,FALSE,FALSE,TRUE;
                    ELSE
                        saveArticleData filename,data,header,MAX_ARTICLE_DESCRIPTION_DATA,FALSE,FALSE,FALSE;
                    ENDIF
                ELSE
                    IF (j=MAX_ARTICLE_COUNTER) THEN
                        saveArticleData filename,data,header,MAX_ARTICLE_DESCRIPTION_DATA,FALSE,FALSE,TRUE;
